...
class GstSummaryReportViewTest(TestCase):
    ...
    def test_admin_can_view_gst_summary_report(self):
        self.client.force_login(self.admin)

        response = self.client.get(reverse("billing:gst_summary_report"))

        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "GST Summary Report")
        self.assertContains(response, "Amoxicillin")
        self.assertContains(response, "3004")

    def test_admin_can_export_gst_summary_csv(self):
        self.client.force_login(self.admin)

        response = self.client.get(
            reverse("billing:gst_summary_report"),
            {"export": "csv"},
        )

        self.assertEqual(response.status_code, 200)
        self.assertEqual(response["Content-Type"], "text/csv")
        self.assertIn("Medicine,HSN Code,GST %,Quantity Sold,Taxable Value,CGST,SGST,Total GST,Gross Total", response.content.decode("utf-8"))
